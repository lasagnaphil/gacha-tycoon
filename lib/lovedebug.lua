...
_Debug.onTop = function()
	...
	if p then
		...
		for i,v in ipairs(p) do
			if v[1] then love.graphics.print(v[1], 5, 2+ 5*(i-1) + (i-1)*_Debug.Font:getHeight()) end
		end
	end
	if e then
		--draw errors
		...
		for i,v in ipairs(e) do
			if v[1] then love.graphics.print(v[1], love.graphics.getWidth()/2+5, 2+ 5*(i-1) + (i-1)*_Debug.Font:getHeight()) end
		end
	end
